...
The in-window dashboard is the main UI. It shows local and peer services, fixed port ownership, health/status, local/network URLs, logs/status commands, and service-level action buttons for `Start`, `Stop`, `Restart`, `Open`, and `Rename`. Owner/local/remote context is shown as badges; the primary action follows whether the service is running, not where it lives. Service rows support local star pinning and toolbar sort modes, with pinned services kept first. External service status prefers the current fixed-port listener over stale handoff PIDs, so Windows wrapper processes do not hide the real service state. It refreshes on startup, manual refresh, window focus/visibility return, and after actions; it does not poll peer machines every few seconds. The top toolbar also includes one `Copy <machine>` handoff prompt button for the local machine and each configured peer, such as `Copy workstation` and `Copy gpu-box`.
